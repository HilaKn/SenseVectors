...
def output_clusters(final_labeling):
    ...
    np.savetxt("{}/{}/{}".format(output_folder,adj_clusters_folder,adj),labels_matrix)
...
if __name__ == '__main__':
    ...
    for filename in adj_files:#for filename in ['blue','hot','long','right','cold','clean','dark','light']:#os.listdir(ADJ_DIC_FOLDER):
        ...
        if len(unique_contexts) < MIN_UNIQUE_ITEMS_FOR_CLUSTERING:
            print "only [{}] unique contexts for [{}]. moving to next adj".format(len(unique_contexts),adj)
            continue
        ...
        try:
            ...
            if k>2 or (k==2 and -1 not in label_to_avg.keys()) :
                output_clusters(final_labeling)
        ...
        for i in xrange(0,len(filtered_contexts_list)):
            ...
            if final_labeling[i] != -1:#update label only for the clustered
