import os
from multi_sense_we_wrapper_v7 import MultiSenseWE
...
def test():
    ...
    x_test = np.array([np.concatenate((we_wrapper.adj_vec_by_context(samp.adj,samp.noun), we_wrapper.word_vec(samp.noun))) for samp in filtered_test_samp])
...
if __name__ == '__main__':
    ...
    we_wrapper = MultiSenseWE(args.we_file,args.adj_with_sense_file,args.adj_clusters_folder)
    ...
    x_train = np.array([np.concatenate((we_wrapper.adj_vec_by_context(samp.adj,samp.noun),we_wrapper.word_vec(samp.noun))) for samp in filtered_samp])
