...
if __name__ == '__main__':
    ...
    for filename in adj_to_cluster:#for filename in ['blue','hot','long','right','cold','clean','dark','light']:#os.listdir(ADJ_DIC_FOLDER):
        ...
        try:
            print "DBSCAN clustering [{}]".format(adj)
            first_clustering = DBSCAN(eps = 0.4,min_samples=5, metric='cosine', algorithm='brute',  n_jobs=10).fit(clustering_input)
            ...
            if k<2:
                print "No clusters or just 1 cluster found in first DBSCAN. skipping adjective [{}]".format(adj)
                continue
