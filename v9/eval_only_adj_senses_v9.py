import os
from multi_sense_we_wrapper_v9 import MultiSenseWE
...
def test():
    ...
    filtered_test_samp = [samp for samp in test_triplets
                          if samp.adj in we_wrapper.vocab and samp.noun in we_wrapper.vocab and samp.attr in we_wrapper.vocab][0:2]
    print "after filter missing words, testing samples: " + str(len(filtered_test_samp))

    x_test = np.array([we_wrapper.all_adj_vecs(samp.adj) for samp in filtered_test_samp])
    ...
    x_test_matrix = np.tensordot(x_test,np.transpose(weights),)# np.dot(weights, np.transpose(x_test))
...
if __name__ == '__main__':
    ...
    filtered_samp = [samp for samp in dev_triplets
                     if samp.adj in we_wrapper.vocab and samp.noun in we_wrapper.vocab and samp.attr in we_wrapper.vocab ][0:10]
