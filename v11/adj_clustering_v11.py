...
if __name__ == '__main__':
    ...
    for filename in ['blue','hot]']:#os.listdir(ADJ_DIC_FOLDER):
        ...
        try:
            k=2
            step = 25000
            print "clustering [{}] for [{}] clusters".format(adj,k)
            for i in xrange(0,clustering_input.shape[0],step):
                limit = i+step
                if i+step >= clustering_input.shape[0]:
                    limit = clustering_input.shape[0]
                print "clustering from {} to {}".format(i,limit)
                clustering_alg = AgglomerativeClustering(n_clusters=k,affinity='cosine',linkage='average').fit(clustering_input[i:limit,:])
        ...
        for i in xrange(0,len(filtered_contexts_list)):
            ...
            context.update_label( clustering_alg.labels_[i])
