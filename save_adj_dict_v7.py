...
NON_RELEVANT_HEAD = ['.',',']
ADJ_DIC_FOLDER = "adj_dic_folder_pickle"
...
if __name__ == '__main__':
    ...
    if not os.path.exists(ADJ_DIC_FOLDER):
        os.makedirs(ADJ_DIC_FOLDER)
    ...
    print "Saving objects to file using json"
    for adj,contexts in adj_dic.iteritems():
        with open(ADJ_DIC_FOLDER + '/' + adj,'wb') as f:
            jason_adj_contexts = jsonpickle.encode(contexts,f)
            json.dump(jason_adj_contexts,f)
    # with open(ADJ_DIC_FILE, 'wb') as f1:
    #     jason_adj_dic = jsonpickle.encode(adj_dic,f1)
    #     json.dump(jason_adj_dic,f1)
        # jason_sentences_dic = jsonpickle.encode(sentence_to_adj_context,f2)
        # json.dump(jason_sentences_dic,f2)

        # cPickle.dump(adj_dic, f1,protocol=2)
        # cPickle.dump(sentence_to_adj_context, f2,protocol=2)