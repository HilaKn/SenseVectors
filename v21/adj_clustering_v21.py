...
if __name__ == '__main__':
    ...
    for filename in ['blue','hot','long','right','cold','clean','dark','light']:#os.listdir(ADJ_DIC_FOLDER):
        ...
        try:
            ...
            clustering_alg = DBSCAN(eps = 0.4,min_samples=5, metric='cosine', algorithm='brute',  n_jobs=-1).fit(clustering_input)
            k_1 = len(list(set(clustering_alg.labels_)))
            ...
            outlier_input = [clustering_input[i] for i in outlier_idx]
            print "DBSCAN clustering [{}] outliers".format(len(outlier_input))
            clustering_alg_2 = DBSCAN(eps = 0.5,min_samples=5, metric='cosine', algorithm='brute',  n_jobs=-1).fit(outlier_input)
            k_2 = len(list(set(clustering_alg_2.labels_)))
            ...
            if (args.analyze_adj_clusters):
                ...
                output_clusters(final_k,final_labeling)
