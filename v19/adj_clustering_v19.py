...
#apply lemmatiazer on the nouns (e.g. consider 'car' and 'cars' as the same noun
TOKEN_ID_COLUMN = 0
TOKEN_COLUMN = 1
POS_COLUMN = 3
DEP_ID_COLUMN = 6
...
if __name__ == '__main__':
    ...
    for filename in ['blue','hot','long','right']:#os.listdir(ADJ_DIC_FOLDER):
        ...
        try:
            ...
            clustering_alg = DBSCAN(eps = 0.5,min_samples=5, metric='cosine', algorithm='brute',  n_jobs=-1).fit(clustering_input)
            k = len(list(set(clustering_alg.labels_)))
            print "done clustering [{}] with [{}] clusters".format(adj,k)


            final_labeling = [clustering_alg.labels_[data_mapper[i]] for i in xrange(0,len(filtered_contexts_list))]
