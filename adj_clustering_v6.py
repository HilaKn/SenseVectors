...
import pickle
from save_adj_dict_v6 import AdjContext
import save_adj_dict_v6 as s
import json
import jsonpickle
...
if __name__ == '__main__':
    ...
    with open(s.ADJ_DIC_FILE, 'rb') as f1:
    ...
    with open(output_folder+"/filtered_adj_dic", 'w') as f:
        ...
        for adj,contexts_list in filtered_adj_dic.iteritems():
            sorted_contexts_list = sorted(contexts_list, key=lambda x: x.head_noun, reverse=True)
            ...
            f.write("{}\t{}\n".format(adj, len(filtered_contexts_list)))
            print "Clustering adj: [{}] with [{}] contexts. filtered [{}]".format(adj, len(contexts_list),len(filtered_contexts_list))
